...
void id() {
  ...
  if (Always && EXIST(balls)) {
    if (DX(balls[0]) * VX(balls[0]) < 0 && (STATE(balls[0]) == 3000 || STATE(balls[0]) >= 3005)) {
      ...
      if ((ABSRANGE(balls[0], 80, 14) && abs(DY(balls[0])) <= 100) || (VZ(balls[0]) != 0 && ABSRANGE(balls[0], 100, 20)))
        D();
      else if ((self.state <= 1 || self.state == 5) && (ABSRANGE(balls[0], 160, 14) && STATE(balls[0]) == 3000) && abs(DX(balls[0])) >= 100)
        A();
  ...
  if (Sometimes) {
    //Jump_sword1.
    if (((EXIST(weapons) && HITABLE(weapons[0], 60)) || STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16 || (self.frame == 110 && self.bdefend > 0)) && HITABLE(enemies[0], 262) && BLINK(enemies[0]) == 0 && self.mp >= 75 && !specialmove && !warning)
      DdA();
    //Blasts.
    if (self.fall == 0 && (RANGEX(enemies[0], -140, -30) || RANGEX(enemies[0], 30, 140)) && ((DZ(enemies[0]) == 0 || abs(DX(enemies[0])) / abs(DZ(enemies[0])) >= mindashx / mindashz) || abs(DZ(enemies[0])) <= 14) && abs(DZ(enemies[0])) <= 30 && self.state <= 1 && self.mp >= 150 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 14 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlA();
      else
    ...
    //Jump_sword2.
    if ((Hardly && self.fall >= 10 && ABSRANGE(enemies[0], 60, 14) && FALL(enemies[0]) == 0) || self.frame == 265 && self.mp >= 75)
      DuJ();
    if (self.frame == 267 && (HITABLE(enemies[0], 269) || HITABLE(enemies[0], 270) || HITABLE(enemies[0], 271)))
      A();
    //Dash_sword.
    if (RANGESF(102, 107) && ABSRANGE(enemies[0], 80, 10) && self.mp >= 150 && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0) DlJ();
      else DrJ();
    }
  }

  //Special direction.
  if ((RANGEX(enemies[0], -160, -30) || RANGEX(enemies[0], 30, 160)) && abs(DZ(enemies[0])) <= 30 && RANGESF(235, 250)) {
    specialmove = true;
  ...
  if (Always && !warning && !specialmove) {
    ...
    if (abs(DX(enemies[0])) <= 80) {
      if (abs(DZ(enemies[0])) <= 25 && (RANGETF(enemies[0], 20, 29) || RANGETF(enemies[0], 35, 39) || (RANGETF(enemies[0], 60, 94) && ID(enemies[0]) != 4 && ID(enemies[0]) != 5))) {
        up(0, 0);
        down(0, 0);
