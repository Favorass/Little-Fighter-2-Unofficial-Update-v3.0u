...
void id() {
  ...
  if (Sometimes) {
    ...
    if (((EXIST(weapons) && HITABLE(weapons[0], 60)) || STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16 || (self.frame == 110 && self.bdefend > 0)) && HITABLE(enemies[0], 271) && BLINK(enemies[0]) == 0 && self.mp >= 75 && !specialmove && !warning)
      DdA();
    //Balls.
    ...
    //Jumphit.
    if ((self.fall >= 10 && ABSRANGE(enemies[0], 60, 14) && FALL(enemies[0]) == 0) || RANGESF(281, 282) && STATE(enemies[0]) == 12 && self.mp >= 25)
      DuJ();
    if (self.frame == 292 && HITABLE(enemies[0], 293))
      A();
    //Singlong.
    if (Hardly && limit <= 1 && (self.state <= 2 || RANGESF(102, 107)) && HITABLE(enemies[0], 301) && abs(DZ(enemies[0])) <= 7 && self.mp >= 225 && STATE(enemies[0]) != 7 && BLINK(enemies[0]) == 0) {
      DuA();
      limit++;
    }
  }
