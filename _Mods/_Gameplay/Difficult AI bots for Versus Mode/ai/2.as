...
void id() {
  ...
  for (uint i = 0; i < 400; i++) {
    if (game.exists[i])
      if (ID(i) == 200 && RANGETF(i, 60, 65) && TEAM(i) == self.team) {
        shield = i;
        break;
      }
  }
  ...
  if (Always && EXIST(balls)) {
    if (DX(balls[0]) * VX(balls[0]) < 0 && (STATE(balls[0]) == 3000 || STATE(balls[0]) >= 3005)) {
      ...
      if ((ABSRANGE(balls[0], 80, 14) && abs(DY(balls[0])) <= 100) || (VZ(balls[0]) != 0 && ABSRANGE(balls[0], 100, 20)))
        D();
      else if ((self.state <= 1 || self.state == 5) && (ABSRANGE(balls[0], 140, 14) && STATE(balls[0]) == 3000))
        A();
      ...
      if (!ISFACING(enemies[0]))
        ...
        down(1, 1);
      if (ABSRANGE(balls[0], 55, 18) && ISFACING(enemies[0]))
        D();
  ...
  if (Sometimes) {
    //Heal self.
    if (Hardly && limit <= 1 && !ABSRANGE(enemies[0], 350, 60) && self.state <= 1 && self.dark_hp - self.hp >= 100 && self.mp >= 350 && !specialmove && !warning) {
      DdJ();
      limit++;
    }
    //Balls.
    if ((RANGEX(enemies[0], -200, -120) || RANGEX(enemies[0], 120, 200)) && abs(DZ(enemies[0])) <= 20 && self.state <= 1 && self.mp >= 75 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 14 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlA();
      else
    ...
    //Force field.
    if (((STATE(enemies[0]) == 8 || (shield == -1 && EXIST(weapons) && HITABLE(weapons[0], 60))) && (HITABLE(enemies[0], 274) || HITABLE(enemies[0], 275))) && self.state <= 1 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 7 && STATE(enemies[0]) != 14 && self.mp >= 100 && !specialmove && !warning)
      if (DX(enemies[0]) < 0)
        DlJ();
      else
        DrJ();
    if (shield == -1 && warning && self.state <= 1 && self.fall == 0 && self.blink == 0 && !((STATE(enemies[0]) == 5 || RANGETF(enemies[0], 90, 94)) && ISFACING(enemies[0])) && self.mp >= 100) {
      if (abs(DY(enemies[0])) <= 40)
        if (DX(enemies[0]) < 0)
          DlJ();
        else
          DrJ();
    ...
    //Biscuit.
    if (chase == -1 && ABSRANGE(enemies[0], 80, 100) && abs(DZ(enemies[0])) >= 40 && self.state <= 1 && self.mp >= 250 && !specialmove && !warning)
      DuA();
  }
  ...
  //Combo.
  if (Often && STATE(enemies[0]) == 12 && (RANGEX(enemies[0], -40, -20) || RANGEX(enemies[0], 20, 40)) && abs(DZ(enemies[0])) <= 14) {
    specialmove = true;
    DASH(enemies[0]);
  }
