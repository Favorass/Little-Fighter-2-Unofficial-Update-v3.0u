...
void id() {
  ...
  if (Always && EXIST(balls)) {
    if (DX(balls[0]) * VX(balls[0]) < 0 && (STATE(balls[0]) == 3000 || STATE(balls[0]) >= 3005)) {
      ...
      if ((ABSRANGE(balls[0], 80, 14) && abs(DY(balls[0])) <= 100) || (VZ(balls[0]) != 0 && ABSRANGE(balls[0], 100, 20)))
        ...
        A();
    } else if (ABSRANGE(balls[0], 300, 30) && ID(balls[0]) == 200 && RANGETF(balls[0], 60, 65) && INFRONT(balls[0])) {
      specialmove = true;
      warning = true;
      if (!ISFACING(balls[0]))
        DODGE(balls[0]);
      ...
      warning = true;
      DODGE(balls[0]);
    } else if (ID(balls[0]) == 212 && ABSRANGE(balls[0], 250, 20) && !RANGETF(balls[0], 150, 170))
      A();
  }
  ...
  if (Always && EXIST(drinks) && self.team == 1 && !warning && !specialmove && mode == 1) {
    ...
    if (choice != -1) {
      ...
      if (abs(DX(drinks[choice])) >= 300 && self.state == 2)
        J();
      else if (ABSRANGE(drinks[choice], 120, 14) && self.state == 2)
        D();
  ...
  //Get far from enemy.
  if (Often && ((self.blink <= 14 && coming && !RANGETF(enemies[0], 55, 58) && !warning) || (STATE(enemies[0]) == 14 && !coming) || (BLINK(enemies[0]) != 0 && self.blink == 0 && !RANGETF(enemies[0], 55, 58)) || (self.weapon_type == 6 && (self.hp <= self.max_hp / 2 || self.mp <= 150) && mode == 1))) {
    specialmove = true;
    GETFARFROM(enemies[0]);
  }

  //Punch.
  if (Sometimes && ABSRANGE(enemies[0], 500, 30) && (abs(DX(enemies[0])) >= 200 || (abs(DX(enemies[0])) >= 150 && STATE(enemies[0]) == 16)) && abs(DY(enemies[0])) <= 40 && self.state <= 1 && self.fall <= 10 && STATE(enemies[0]) != 13 && STATE(enemies[0]) != 14 && INFRONT(enemies[0]) && BLINK(enemies[0]) == 0 && self.weapon_type == 0 && !warning && !specialmove && !(coming && !RANGETF(enemies[0], 55, 58) && self.blink == 0)) {
    A();
  }
  if (STATE(enemies[0]) == 8 && self.state <= 1 && ABSRANGE(enemies[0], 80, 14)) {
  ...
  //Grab.
  if (Always && STATE(enemies[0]) == 16 && ABSRANGE(enemies[0], 120, 30) && !(coming && self.blink == 0)) {
    specialmove = true;
    if (DX(enemies[0]) <= -10)
      left(1, 1);
  ...
  if (Often && self.state == 5 && ABSRANGE(enemies[0], 80, 20) && INFRONT(enemies[0]) && STATE(enemies[0]) != 14 && BLINK(enemies[0]) == 0)
    A();
  if (Often && (self.frame == 84 || self.frame == 214 || self.frame == 217) && STATE(enemies[0]) != 14 && BLINK(enemies[0]) == 0)
    A();

  //Skills.
  if (Sometimes) {
    //Flute.
    if (limit <= 1 && Hardly && self.state <= 1 && !ABSRANGE(enemies[0], 100, 25) && ABSRANGE(enemies[0], 150, 50) && STATE(enemies[0]) != 14 && self.mp >= 350 && !specialmove && !warning) {
      DuJ();
      limit++;
    }
    //Five arrow.
    if (ABSRANGE(enemies[0], 300, 30) && abs(DX(enemies[0])) >= 250 && abs(DY(enemies[0])) <= 50 && self.mp >= 150 && self.fall == 0 && INFRONT(enemies[0]) && STATE(enemies[0]) != 14 && BLINK(enemies[0]) == 0 && !specialmove && !warning) {
      DJA();
    }
    //Blastpush.
    if (STATE(enemies[0]) == 16 && self.fall == 0 && (RANGEX(enemies[0], -200, -100) || RANGEX(enemies[0], 100, 200)) && abs(DZ(enemies[0])) <= 14 && self.state <= 1 && self.mp >= 150 && BLINK(enemies[0]) == 0 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlA();
      else
    ...
    //Super arrow.
    if (Hardly && self.fall == 0 && (RANGEX(enemies[0], -400, -350) || RANGEX(enemies[0], 350, 400)) && abs(DZ(enemies[0])) <= 40 && self.state <= 1 && self.mp >= 200 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 12 && STATE(enemies[0]) != 14 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlJ();
      else
  ...
  //Special direction.
  if ((RANGEX(enemies[0], -400, -30) || RANGEX(enemies[0], 30, 400)) && (RANGESF(60, 66) || RANGESF(80, 84) || RANGESF(270, 287))) {
    specialmove = true;
    if (DZ(enemies[0]) <= -14)
      up(1, 1);
    else if (DZ(enemies[0]) >= 14)
  ...
  if (Often && (STATE(enemies[0]) == 12 || RANGETF(enemies[0], 100, 109)) && abs(DY(enemies[0])) <= 60 && self.state <= 1 && INFRONT(enemies[0]) && BLINK(enemies[0]) == 0)
    if ((abs(DX(enemies[0])) >= 100 && abs(DX(enemies[0])) <= 140 && abs(DY(enemies[0])) <= 60) || (abs(DX(enemies[0])) >= 50 && abs(DX(enemies[0])) <= 100 && abs(DY(enemies[0])) <= 30))
      A();

  //Dash2.
  if (Sometimes && ABSRANGE(enemies[0], 250, 50) && self.state <= 2 && STATE(enemies[0]) != 16 && !specialmove && !warning) {
    specialmove = true;
    if (rand(5) < 3)
      left(1, 0);
    else
  ...
  if (Always && !warning && !specialmove) {
    if (Sometimes && abs(DX(enemies[0])) >= 250 && self.state <= 2) {
      if (DX(enemies[0]) <= 0)
        left(1, 1);
      else
        right(1, 1);
      if (self.state == 2 && (DX(enemies[0])) > maxdashx * 4)
        J();
    }
    if (abs(DX(enemies[0])) <= 600) {
      if (abs(DZ(enemies[0])) <= 20 && (RANGETF(enemies[0], 20, 29) || RANGETF(enemies[0], 35, 39))) {
        up(0, 0);
        down(0, 0);
  ...
  if (Sometimes) {
    switch (game.objects[enemies[0]].data.id) {
      ...
      {
        if ((RANGEX(enemies[0], -300, -200) || RANGEX(enemies[0], 200, 300)) && abs(DZ(enemies[0])) <= 14 && self.mp >= 150 && BLINK(enemies[0]) == 0 && RANGETF(enemies[0], 290, 309)) {
          specialmove = true;
          warning = true;
          if (DX(enemies[0]) < 0)
            DlA();
      ...
      {
        if (RANGETF(enemies[0], 60, 62) && ABSRANGE(enemies[0], 200, 30) && ISFACING(enemies[0]) && self.state <= 1)
          A();
      ...
      {
        if ((RANGEX(enemies[0], -300, -200) || RANGEX(enemies[0], 200, 300)) && abs(DZ(enemies[0])) <= 14 && self.mp >= 150 && BLINK(enemies[0]) == 0 && RANGETF(enemies[0], 255, 261)) {
          specialmove = true;
          warning = true;
          if (DX(enemies[0]) < 0)
            DlA();
      ...
      {
        if ((RANGEX(enemies[0], -300, -200) || RANGEX(enemies[0], 200, 300)) && abs(DZ(enemies[0])) <= 14 && self.mp >= 150 && BLINK(enemies[0]) == 0 && RANGETF(enemies[0], 280, 290)) {
          specialmove = true;
          warning = true;
          if (DX(enemies[0]) < 0)
            DlA();
...
void GETFARFROM(int i) {
  float maxdashx = 11 * self.data.dash_distance;
  if (ABSRANGE(i, 300, 300) && self.state != 5) {
    if (self.x >= maxdashx && self.x <= bg_width - maxdashx) {
      if (DX(i) > 0)
        left(1, 0);
      else if (DX(i) < 0)
