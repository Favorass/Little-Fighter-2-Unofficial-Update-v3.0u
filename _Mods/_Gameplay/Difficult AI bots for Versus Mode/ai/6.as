...
void id() {
  ...
  float maxdashx = 8 * self.data.dash_distance;
  float mindashz = 6 * self.data.dash_distancez;
  float maxdashz = 15 * self.data.dash_distancez;
  ...
  //Hit armour.
  if (self.bdefend > 0 && self.state != 7 && self.state != 8)
    hitarmour = true;

  //Transform.
  if (self.hp <= self.max_hp / 3 && (!ABSRANGE(enemies[0], 250, 40) || warning))
    DJA();

  //Dodging,defense or rebound ball.
  if (Always && EXIST(balls)) {
    if (DX(balls[0]) * VX(balls[0]) < 0 && (STATE(balls[0]) == 3000 || STATE(balls[0]) >= 3005)) {
      ...
      if ((ABSRANGE(balls[0], 80, 14) && abs(DY(balls[0])) <= 100) || (VZ(balls[0]) != 0 && ABSRANGE(balls[0], 100, 20)))
        D();
      else if ((self.state <= 1 || self.state == 5) && (ABSRANGE(balls[0], 180, 20) && !ABSRANGE(balls[0], 170, 20) && STATE(balls[0]) == 3000))
        A();
      ...
      warning = true;
      DODGE(balls[0]);
    } else if (ID(balls[0]) == 212 && HITABLE(balls[0], 61) && !RANGETF(balls[0], 150, 170))
      A();
  }
  ...
  //Dodging jump attack.
  if (Always && hitarmour && ABSRANGE(enemies[0], 80, 40) && abs(DY(enemies[0])) >= 20 && !GOINGTOBEHITED(enemies[0]) && (STATE(enemies[0]) == 3 || STATE(enemies[0]) == 4) && ID(enemies[0]) != 4 && self.blink == 0) {
    specialmove = true;
    warning = true;
    DODGE2(enemies[0]);
  }
  ...
  //Dodging dash attack.
  if (Always && hitarmour && ABSRANGE(enemies[0], 250, 40) && (STATE(enemies[0]) == 5 || RANGETF(enemies[0], 40, 44) || RANGETF(enemies[0], 90, 94)) && INFRONT(enemies[0]) && self.blink == 0) {
    specialmove = true;
    warning = true;
    DODGE(enemies[0]);
  }
  ...
  //Defense.
  if (Always && (GOINGTOBEHITED(enemies[0]) || (coming && GOINGTOBEHITED(enemies[1]))) && hitarmour && self.state <= 1 && AREST(enemies[0]) == 0 && self.blink == 0) {
    specialmove = true;
    warning = true;
  ...
  //Throwing weapon.
  if (Always && self.weapon_type != 0 && self.weapon_type != 1 && !(self.weapon_type == 6 && (self.hp <= self.max_hp / 2 || self.mp <= 150) && mode == 1)) {
    specialmove = true;
    if (DX(enemies[0]) <= 0)
      left(1, 0);
    else
  ...
  //Punch.
  if (Reverse && HITABLE(enemies[0], 61) && self.fall <= 10 && !(STATE(enemies[0]) == 7 && FALL(enemies[0]) == 0 && (ID(enemies[0]) == 1 || ID(enemies[0]) == 9 || ID(enemies[0]) == 10 || ID(enemies[0]) == 11)) && STATE(enemies[0]) != 13 && STATE(enemies[0]) != 14 && STATE(enemies[0]) != 16 && !(RANGETF(enemies[0], 20, 29) || RANGETF(enemies[0], 35, 39) || (RANGETF(enemies[0], 60, 94) && ID(enemies[0]) != 4 && ID(enemies[0]) != 5)) && BLINK(enemies[0]) == 0 && !warning && !specialmove && !(coming && !RANGETF(enemies[0], 55, 58) && self.blink == 0)) {
    if (!INFRONT(enemies[0]))
      TURN();
    A();
  }
  if (Sometimes && HITABLE(enemies[0], 61) && self.fall <= 10 && !(STATE(enemies[0]) == 7 && FALL(enemies[0]) == 0) && STATE(enemies[0]) != 13 && STATE(enemies[0]) != 14 && STATE(enemies[0]) != 16 && BLINK(enemies[0]) == 0 && ((hitarmour && AREST(enemies[0]) != 0) || FALL(enemies[0]) > 0) && !warning && !specialmove && !(coming && !RANGETF(enemies[0], 55, 58) && self.blink == 0)) {
    if (!INFRONT(enemies[0]))
      TURN();
    A();
  }
  if (RANGETF(enemies[0], 110, 111) && FALL(enemies[0]) == 0 && BDEFEND(enemies[0]) > 0 && (ID(enemies[0]) == 1 || ID(enemies[0]) == 9 || ID(enemies[0]) == 10 || ID(enemies[0]) == 11))
    DODGE(enemies[0]);
  if (EXIST(weapons) && HITABLE(weapons[0], 60) && !HITABLE(enemies[0], 60) && self.weapon_type == 0)
    A(1, 0);
  ...
  //Grabbing.
  if (Always && self.frame == 121 && self.ctimer <= 10 && self.mp >= 75) {
    specialmove = true;
    J();
  }
  if (Always && HP(enemies[0]) > 0 && (RANGESF(121, 123) || RANGESF(232, 234))) {
    specialmove = true;
    if ((self.ctimer <= 10 && self.hp > 30) || (coming && self.blink == 0)) {
      if (self.x >= bg_width / 2)
        left(1, 1);
      else
        right(1, 1);
    } else if (self.ctimer >= 10) {
      left(0, 0);
      right(0, 0);
    }
    if (self.ctimer >= 60)
  ...
  //Dash attack.
  if (Hardly && abs(DX(enemies[0])) >= mindashx && abs(DX(enemies[0])) <= maxdashx && ((abs(DZ(enemies[0])) >= mindashz && abs(DZ(enemies[0])) <= maxdashz) || abs(DZ(enemies[0])) <= 14) && ((DZ(enemies[0]) == 0 || abs(DX(enemies[0])) / abs(DZ(enemies[0])) >= mindashx / mindashz) || abs(DZ(enemies[0])) <= 14) && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 14 && STATE(enemies[0]) != 16 && self.mp >= 75 && !warning) {
    specialmove = true;
    DASH(enemies[0]);
  }
  if (Hardly && STATE(enemies[0]) == 13 && self.mp >= 75 && !warning) {
    specialmove = true;
    DASH(enemies[0]);
  }
  if (Often && !hitarmour && self.state == 5 && ABSRANGE(enemies[0], 120, 30) && INFRONT(enemies[0]) && STATE(enemies[0]) != 14 && BLINK(enemies[0]) == 0)
    A();

  //Counter running attack or fake falling.
  if (Often && abs(DX(enemies[0])) <= 80 && abs(DZ(enemies[0])) <= 14 && self.mp >= 50 && !specialmove && !warning) {
    specialmove = true;
    if ((STATE(enemies[0]) == 5 || RANGETF(enemies[0], 90, 94)) && self.state != 2) {
      if (FACING(enemies[0]) == true)
        left(1, 0);
      else
        right(1, 0);
    }
  ...
  if (Sometimes) {
    //Blastpush.
    if (ABSRANGE(enemies[0], 350, 14) && abs(DX(enemies[0])) >= 300 && self.state <= 1 && self.mp >= 150 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 14 && !specialmove && !warning) {
      if (DX(enemies[0]) < 0)
        DlA();
      else
        DrA();
    ...
    //C-throw.
    if (INFRONT(enemies[0]) && (STATE(enemies[0]) == 8 || (hitarmour && abs(DY(enemies[0])) <= 20 && FALL(enemies[0]) == 0)) && ABSRANGE(enemies[0], 50, 14) && STATE(enemies[0]) != 14 && BLINK(enemies[0]) == 0 && (self.state <= 1 || self.state == 7) && self.fall == 0 && self.mp >= 75)
      DuJ();
    if (RANGESF(261, 270) && self.mp > 250)
      J();
    //1000foot.
    if (((hitarmour && !ISFACING(enemies[0])) || STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16) && abs(DX(enemies[0])) >= 60 && abs(DX(enemies[0])) <= 120 && abs(DZ(enemies[0])) <= 14 && STATE(enemies[0]) != 14 && BLINK(enemies[0]) == 0 && self.fall == 0)
      if (DX(enemies[0]) <= 0)
        DlJ();
      else
        DrJ();
    //Prevent enemy rowing.
    if (RANGETF(enemies[0], 100, 109) && DY(enemies[0]) <= 40 && abs(DZ(enemies[0])) <= 14 && self.state <= 1 && BLINK(enemies[0]) == 0) {
      if (abs(DX(enemies[0])) >= 100 && abs(DX(enemies[0])) <= 120 && abs(DY(enemies[0])) <= 30) {
        if (DX(enemies[0]) <= 0)
          DlJ();
        else
          DrJ();
      } else if (abs(DX(enemies[0])) >= 50 && abs(DX(enemies[0])) <= 100 && abs(DY(enemies[0])) <= 20) {
        if (DX(enemies[0]) <= 0)
          DlJ();
        else if (DX(enemies[0]) <= 0)
          DrJ();
      } else if (abs(DX(enemies[0])) < 50 && abs(DY(enemies[0])) <= 20) {
        if (!ISFACING(enemies[0]))
          TURN();
        A();
      }
  ...
  if (Always && !warning && !specialmove) {
    if (abs(DX(enemies[0])) <= 350) {
      if (DX(enemies[0]) <= -10) {
        ...
        if (Sometimes && self.state == 2 && STATE(enemies[0]) <= 1 && !warning)
          left(1, 0);
        right(1, 1);
      } else if (hitarmour && STATE(enemies[0]) <= 1 && FALL(enemies[0]) == 0)
        DODGE(enemies[0]);
    } else {
      if (DX(enemies[0]) <= 0)
        left(1, 0);
    ...
    if (abs(DX(enemies[0])) <= 80) {
      if (abs(DZ(enemies[0])) <= 20 && ((hitarmour && STATE(enemies[0]) <= 1 && FALL(enemies[0]) == 0) || RANGETF(enemies[0], 20, 29) || RANGETF(enemies[0], 35, 39) || (RANGETF(enemies[0], 60, 94) && ID(enemies[0]) != 4 && ID(enemies[0]) != 5))) {
        up(0, 0);
        down(0, 0);
  ...
  if (Sometimes) {
    switch (game.objects[enemies[0]].data.id) {
      ...
      {
        if (Hardly && (RANGEX(enemies[0], -500, -150) || RANGEX(enemies[0], 150, 500))) {
          if (self.z <= bg_zwidth1 + 1.5 * maxdashz || self.z >= bg_zwidth2 - 1.5 * maxdashz) {
            specialmove = true;
            if (DX(enemies[0]) <= 0)
              left(1, 0);
            else
              right(1, 0);
            if (self.z >= bg_zwidth2 - 1.5 * maxdashz)
              up(1, 1);
            else if (self.z <= bg_zwidth1 + 1.5 * maxdashz)
              down(1, 1);
            if (STATE(enemies[0]) != 5 && (self.state == 2 || self.frame == 215))
              J();
          } else if (abs(DZ(enemies[0])) >= 40)
            DASH(enemies[0]);
          else if (abs(DZ(enemies[0])) < 40)
            DODGE(enemies[0]);
        }
