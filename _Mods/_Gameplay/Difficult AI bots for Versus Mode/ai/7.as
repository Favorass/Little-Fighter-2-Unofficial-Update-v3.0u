...
void id() {
  ...
  if (Always && EXIST(balls)) {
    if (DX(balls[0]) * VX(balls[0]) < 0 && (STATE(balls[0]) == 3000 || STATE(balls[0]) >= 3005)) {
      ...
      if ((ABSRANGE(balls[0], 80, 14) && abs(DY(balls[0])) <= 100) || (VZ(balls[0]) != 0 && ABSRANGE(balls[0], 100, 20)))
        D();
      else if ((self.state <= 1 || self.state == 5) && (ABSRANGE(balls[0], 120, 14) && STATE(balls[0]) == 3000))
        A();
  ...
  //Defense.
  if (Always && (GOINGTOBEHITED(enemies[0]) || (coming && GOINGTOBEHITED(enemies[1]))) && STATE(enemies[0]) != 18 && self.blink == 0) {
    specialmove = true;
    warning = true;
  ...
  //Punch.
  if (Sometimes && ABSRANGE(enemies[0], 70, 25) && self.state <= 2 && !(self.fall > 10 && FALL(enemies[0]) == 0) && !(STATE(enemies[0]) == 7 && FALL(enemies[0]) == 0 && (ID(enemies[0]) == 1 || ID(enemies[0]) == 9 || ID(enemies[0]) == 10 || ID(enemies[0]) == 11)) && STATE(enemies[0]) != 8 && STATE(enemies[0]) != 13 && STATE(enemies[0]) != 14 && STATE(enemies[0]) != 16 && BLINK(enemies[0]) == 0 && self.weapon_type == 0 && !warning && !specialmove && !(coming && !RANGETF(enemies[0], 55, 58) && self.blink == 0)) {
  ...
  if (Sometimes) {
    //Explosion.
    if ((STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16) && ABSRANGE(enemies[0], 30, 37) && self.state <= 1 && self.hp >= 40 && self.mp >= 300)
      DuJ();
    //Balls.
    if ((RANGEX(enemies[0], -120, -70) || RANGEX(enemies[0], 70, 120)) && self.state <= 1 && abs(DZ(enemies[0])) <= 20 && BLINK(enemies[0]) == 0 && self.fall == 0 && self.mp >= 75 && !specialmove && !warning) {
      if (DX(enemies[0]) < 0)
        DlA();
      else
        DrA();
    ...
    //Flame.
    if (abs(VX(enemies[0])) >= 7 && abs(DZ(enemies[0])) <= 40 && ISFACING(enemies[0]))
      if ((VX(enemies[0]) <= 0 && RANGEX(enemies[0], 250, 350)) || VX(enemies[0]) >= 0 && RANGEX(enemies[0], -350, -200))
        if ((VZ(enemies[0]) < 0 && DZ(enemies[0]) > 0) || (VZ(enemies[0]) > 0 && DZ(enemies[0]) < 0) || (VZ(enemies[0]) == 0 && abs(DZ(enemies[0])) <= 14))
          DdJ();
    if (self.frame == 275)
      D(1, 0);
    //burn_run.
    if (EXIST(weapons) && HITABLE(weapons[0], 60) && ABSRANGE(enemies[0], 90, 25) && self.state <= 1 && BLINK(enemies[0]) == 0 && self.fall == 0 && self.mp >= 75 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlJ();
      else
  ...
  //Combo.
  if (Sometimes && difficulty == -1 && (STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16) && ABSRANGE(enemies[0], 10, 14)) {
    specialmove = true;
  ...
  if (RANGESF(257, 261) && STATE(enemies[0]) != 18)
    J();
  if (difficulty == -1 && (STATE(enemies[0]) == 11 || STATE(enemies[0]) == 18) && ABSRANGE(enemies[0], 80, 14) && self.hp > 30) {
    specialmove = true;
    if (DX(enemies[0]) <= -52)
      left(1, 1);
    else if (DX(enemies[0]) >= 52)
      right(1, 1);
    if (ABSRANGE(enemies[0], 60, 14))
      if (DZ(enemies[0]) <= 0)
        up(1, 1);
      else
        down(1, 1);
    if (ABSRANGE(enemies[0], 52, 14) && INFRONT(enemies[0])) {
      left(0, 0);
      right(0, 0);
      up(0, 0);
      down(0, 0);
