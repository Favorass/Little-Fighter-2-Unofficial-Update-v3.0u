...
void id() {
  ...
  if (Always && EXIST(balls)) {
    if (DX(balls[0]) * VX(balls[0]) < 0 && (STATE(balls[0]) == 3000 || STATE(balls[0]) >= 3005)) {
      ...
      if ((ABSRANGE(balls[0], 80, 14) && abs(DY(balls[0])) <= 100) || (VZ(balls[0]) != 0 && ABSRANGE(balls[0], 100, 20)))
        D();
      else if ((self.state <= 1 || self.state == 5) && ((sword && ABSRANGE(balls[0], 200, 30)) || ABSRANGE(balls[0], 120, 14)) && STATE(balls[0]) == 3000)
        A();
  ...
  //Dodging dash attack.
  if (Always && ABSRANGE(enemies[0], 250, 40) && (STATE(enemies[0]) == 5 || RANGETF(enemies[0], 40, 44) || RANGETF(enemies[0], 90, 94)) && INFRONT(enemies[0]) && self.blink == 0) {
    specialmove = true;
    warning = true;
  ...
  for (uint i = 0; i < 400; i++) {
    loadTarget(i);
    if (game.exists[i] && ID(i) == 213 && STATE(i) == 1001 && self.team == TEAM(i) && self.weapon_held == target.num) {
      sword = true;
      break;
    }
  }
  if (Always && self.weapon_type != 0 && !(self.weapon_type == 1 && sword) && !(self.weapon_type == 6 && (self.hp <= self.max_hp / 2 || self.mp <= 150) && mode == 1)) {
    specialmove = true;
    if (DX(enemies[0]) <= 0)
      left(1, 0);
    else
  ...
  //Punch.
  if (Sometimes && (ABSRANGE(enemies[0], 70, 25) || (self.weapon_type == 1 && sword && ABSRANGE(enemies[0], 90, 30))) && self.state <= 2 && !(self.fall > 10 && FALL(enemies[0]) == 0) && !(STATE(enemies[0]) == 7 && FALL(enemies[0]) == 0 && (ID(enemies[0]) == 1 || ID(enemies[0]) == 9 || ID(enemies[0]) == 10 || ID(enemies[0]) == 11)) && STATE(enemies[0]) != 8 && STATE(enemies[0]) != 13 && STATE(enemies[0]) != 14 && STATE(enemies[0]) != 16 && BLINK(enemies[0]) == 0 && !warning && !specialmove && !(coming && !RANGETF(enemies[0], 55, 58) && self.blink == 0)) {
  ...
  if (RANGETF(enemies[0], 110, 111) && FALL(enemies[0]) == 0 && BDEFEND(enemies[0]) > 0 && (ID(enemies[0]) == 1 || ID(enemies[0]) == 9 || ID(enemies[0]) == 10 || ID(enemies[0]) == 11))
    DODGE(enemies[0]);
  if (EXIST(weapons) && HITABLE(weapons[0], 60) && !sword)
    A(0, 0);

  //Grab.
  if (Always && STATE(enemies[0]) == 16 && ABSRANGE(enemies[0], 120, 30) && self.mp <= 100 && !(coming && self.blink == 0)) {
    specialmove = true;
    if (DX(enemies[0]) <= -10)
      left(1, 1);
  ...
  if (Sometimes) {
    //Whirlwind.
    if (((Hardly && STATE(enemies[0]) <= 1 && FALL(enemies[0]) == 0 && ABSRANGE(enemies[0], 60, 35) && abs(DZ(enemies[0])) >= 30 && BLINK(enemies[0]) == 0) || (STATE(enemies[0]) == 13 && ABSRANGE(enemies[0], 150, 30))) && self.state <= 1 && self.mp >= 300) {
      DuJ();
    }
    //Balls.
    if ((STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16 || (EXIST(weapons) && HITABLE(weapons[0], 60) && STATE(enemies[0]) != 13)) && (RANGEX(enemies[0], -70, -30) || RANGEX(enemies[0], 30, 70)) && self.state <= 1 && abs(DZ(enemies[0])) <= 20 && BLINK(enemies[0]) == 0 && self.fall == 0 && self.mp >= 100 && !specialmove && !warning) {
      if (DX(enemies[0]) < 0)
        DlA();
      else
        DrA();
    ...
    //Down punch.
    if ((RANGEX(enemies[0], -250, -200) || RANGEX(enemies[0], 200, 250)) && abs(DZ(enemies[0])) >= 20 && abs(DZ(enemies[0])) <= 30 && STATE(enemies[0]) != 12 && STATE(enemies[0]) != 13 && BLINK(enemies[0]) == 0 && self.mp >= 150 && !specialmove && !warning) {
      if (DX(enemies[0]) < 0)
        DlJ();
      else
        DrJ();
    ...
    //Sword.
    if (Hardly && !ABSRANGE(enemies[0], 350, 70) && !sword && self.state <= 1 && self.mp >= 150 && self.blink == 0 && !specialmove && !warning)
      DdJ();
  }
  ...
  //Combo.
  if (Often && (STATE(enemies[0]) == 12 || STATE(enemies[0]) == 13) && DY(enemies[0]) <= -20) {
    specialmove = true;
    if (!sword) {
      if (VY(enemies[0]) >= 6)
        DASH(enemies[0]);
      ...
      if (self.frame == 212)
        A();
      if (Often && self.state == 4 && STATE(enemies[0]) == 12) {
        if (DX(enemies[0]) < 0)
          left(1, 0);
        else
          right(1, 0);
  ...
  if (Always && !warning && !specialmove) {
    if (abs(DX(enemies[0])) <= 400) {
      if (DX(enemies[0]) <= -30) {
        if (Sometimes && self.state == 2 && STATE(enemies[0]) <= 1 && !warning)
          right(1, 0);
        left(1, 1);
      } else if (DX(enemies[0]) >= 30) {
        if (Sometimes && self.state == 2 && STATE(enemies[0]) <= 1 && !warning)
          left(1, 0);
        right(1, 1);
      } else if (((STATE(enemies[0]) == 8 || STATE(enemies[0]) == 16 || (EXIST(weapons) && HITABLE(weapons[0], 60))) && !sword) && abs(DX(enemies[0])) <= 40) {
        if (DX(enemies[0]) >= 0)
          left(1, 1);
        else
          right(1, 1);
      } else if (Hardly && STATE(enemies[0]) <= 1 && abs(DZ(enemies[0])) >= 30 && ID(enemies[0]) != 4 && ID(enemies[0]) != 5)
        GETFARFROM2(enemies[0]);
      else if (Hardly && STATE(enemies[0]) <= 1 && FALL(enemies[0]) == 0 && abs(DZ(enemies[0])) <= 20 && ID(enemies[0]) != 4 && ID(enemies[0]) != 5)
        GETFARFROM(enemies[0]);
    } else {
      if (DX(enemies[0]) <= 0)
        left(1, 0);
