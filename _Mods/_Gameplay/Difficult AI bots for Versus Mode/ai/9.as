...
void id() {
  ...
  //Grab.
  if (Sometimes && STATE(enemies[0]) == 16 && ABSRANGE(enemies[0], 120, 30) && !(coming && self.blink == 0)) {
    specialmove = true;
    if (DX(enemies[0]) <= -10)
      left(1, 1);
  ...
  if (Sometimes) {
    //Many foot.
    if (self.frame == 121 && self.mp >= 75)
      DdA();
    if (((EXIST(weapons) && HITABLE(weapons[0], 60)) || STATE(enemies[0]) == 16 || (self.frame == 110 && self.bdefend > 0)) && HITABLE(enemies[0], 266) && INFRONT(enemies[0]) && BLINK(enemies[0]) == 0 && self.mp >= 75 && !specialmove && !warning)
      DdA();
    //Balls.
    if ((RANGEX(enemies[0], -140, -90) || RANGEX(enemies[0], 90, 140)) && abs(DZ(enemies[0])) <= 30 && self.state <= 1 && self.mp >= 160 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 14 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlA();
      else
    ...
    //C foot.
    if (((STATE(enemies[0]) == 8 && HITABLE(enemies[0], 282)) || (RANGEX(enemies[0], -120, -80) || RANGEX(enemies[0], 80, 120))) && abs(DZ(enemies[0])) <= 17 && BLINK(enemies[0]) == 0 && STATE(enemies[0]) != 7 && STATE(enemies[0]) != 14 && self.state <= 1 && self.mp >= 150 && !specialmove && !warning) {
      specialmove = true;
      if (DX(enemies[0]) < 0)
        DlJ();
      else
    ...
    if (RANGESF(284, 287) && (STATE(enemies[0]) == 14 || STATE(enemies[0]) == 16 || abs(DX(enemies[0])) >= 100))
      D();
    //Chase ball.
    if (RANGESF(295, 301))
      specialmove = true;
    if (chase == -1 && ABSRANGE(enemies[0], 80, 100) && abs(DZ(enemies[0])) >= 70 && self.mp >= 100 && !specialmove && !warning)
      DuA();
  }

  //Special direction.
  if ((RANGEX(enemies[0], -200, -90) || RANGEX(enemies[0], 90, 200)) && abs(DZ(enemies[0])) <= 30 && RANGESF(235, 246)) {
    specialmove = true;
